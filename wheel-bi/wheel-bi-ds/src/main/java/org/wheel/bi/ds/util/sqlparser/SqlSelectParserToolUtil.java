package org.wheel.bi.ds.util.sqlparser;
...
public class SqlSelectParserToolUtil {
    ...
    public static SQLExpr createCondition(SQLExpr columnExpr, DsSqlComparisonOperatorEnum operator, List<Object> values) {
        SQLExpr valueExpr = SQLExprUtils.fromJavaObject(values.get(0), TimeZone.getDefault());
        SQLExpr sqlExpr = null;
        switch (operator) {
            case IN:
            case NOT_IN:
                SQLInListExpr in = new SQLInListExpr(columnExpr, operator == DsSqlComparisonOperatorEnum.IN);
                values.forEach(val -> {
    ...
    public static SQLExpr createConditionGroup(DsSqlTernaryOperatorEnum ternaryOperatorEnum, List<SQLExpr> conditionExpr) {
        ...
        if (ternaryOperatorEnum == DsSqlTernaryOperatorEnum.NOT) {
