...
@Getter
@AllArgsConstructor
public enum DsSqlComparisonOperatorEnum {
    ...
    public static DsSqlComparisonOperatorEnum getEnum(String code) {
        return Arrays.stream(DsSqlComparisonOperatorEnum.values()).filter(node -> StrUtil.equals(node.getCode(), code)).findAny().orElse(EMPTY);
    }
}
