...
@Getter
@AllArgsConstructor
public enum DsSqlTernaryOperatorEnum {
    ...
    public static DsSqlTernaryOperatorEnum getEnum(String code) {
        return Arrays.stream(DsSqlTernaryOperatorEnum.values()).filter(node -> StrUtil.equals(node.getCode(), code)).findAny().orElse(EMPTY);
    }
}
